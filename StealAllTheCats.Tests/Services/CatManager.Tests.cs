...
public class CatManagerTests
{
    ...
    public async Task FetchCatsAsync_ShouldReturnSuccess_WhenCatsAreFetched()
    {
        ...
        catsApiHttpServiceMock.Setup(x => x.GetCatsAsync(1, 25)).ReturnsAsync(Result<IEnumerable<CatApiResponse>>.Ok(catApiResponseList));
        ...
        breedServiceMock.Setup(x => x.ConstructBreeds("Affectionate, Intelligent, Curious, Social, Playful")).ReturnsAsync(Result<IEnumerable<CatTag>>.Ok(new List<CatTag>()));
        
        var loggerMock = new Mock<ILogger<CatManager>>();
        
        var catManager = new CatManager(_catServiceMock.Object, breedServiceMock.Object, catsApiHttpServiceMock.Object, loggerMock.Object);
